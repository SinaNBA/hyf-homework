...
//number of movies
let i = 0;
const numOfMovies = movies.filter(value => value.year >= 1980 && value.year <= 1989)
    .forEach(value => i++);
console.log("The numbers of movies between 1980-1989 is: ", i);

//extra key called tag  Good (>= 7), Average (>= 4 and < 7), Bad (< 4)
const moviesWithTagValue = movies.filter(value => {
    if (value.rating >= 7) {
        return value.tag = "Good"
    } else if (value.rating >= 4 && value.rating < 7) {
        return value.tag = "Average"
    } else {
        return value.tag = "Bad"
    }
});
console.log(moviesWithTagValue);

//Using chaining
const highRated = movies.filter(value => value.rating > 6)
    .map(value => value.rating);
console.log(highRated);

//Count the total number of movies "Surfer, Alien or Benjamin"
let y = 0;
const moviesWithNames = movies.filter(value => {
    let temp = value.title.toLowerCase();
    if (temp.includes("surfer") || temp.includes("alien") || temp.includes("benjamin")) {
        return true;
    } else {
        return false;
    }
});
for (let i = 0; i < moviesWithNames.length; i++) {
    y++;
}
...
//Duplicated titles
let x = 0;
const duplicatedWordsArray = movies.filter(value => {
    let temp = value.title.split(" ");
...
console.log(duplicatedWordsArray);
console.log(x);

//Mostly duplicated
let arrX = [];
const mostlyDuplicated = duplicatedWordsArray.map(value => value.title)
    .forEach(value => {
...
//average rating
const ratingArray = movies.map(value => value.rating);
console.log(ratingArray);

function getSum(total, num) {
    return total + Math.round(num);
};
let sum = ratingArray.reduce(getSum, 0);
let average = Math.round(sum / ratingArray.length);
console.log(average);

//Count the total number of Good, Average and Bad movies
const tagArray = moviesWithTagValue.map(movie => movie.tag);
let countedTags = tagArray.reduce((allTags, tag) => {
    if (tag in allTags) {
        allTags[tag]++;
    } else {
        allTags[tag] = 1;
